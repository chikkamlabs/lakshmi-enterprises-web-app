...
import { getBackorderItems, deleteBackorderItem, BackorderItem } from '../../../../lib/backorderproductStore';
...
function BackorderProductsContent() {
  ...
  const handleToggleSelectAll = () => {
    if (allSelected) {
      ...
      const newIds = Array.from(new Set([...selectedIds, ...filteredItems.map((item) => item.id)]));
  ...
  const handleDeleteSelected = async () => {
    ...
    setDeleting(true);
    try {
      await Promise.all(selectedIds.map((id) => deleteBackorderItem(id)));
      setItems((prev) => prev.filter((item) => !selectedIds.includes(item.id)));
      setSelectedIds([]);
      setShowDeleteConfirm(false);
    } catch (err) {
      console.error('Error deleting backorder items:', err);
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-4 sm:p-6 max-w-7xl mx-auto w-full space-y-6">
          ...
          {/* Controls: Search, Select All Checkbox, Delete Option & Company Filter */}
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
            <div className="flex items-center gap-2.5 flex-1">
              {/* Select All Checkbox beside left to search bar */}
              <div className="flex items-center justify-center pl-1" title={allSelected ? 'Deselect All' : 'Select All'}>
                <input
                  type="checkbox"
                  id="select-all-backorder-checkbox"
                  checked={allSelected}
                  onChange={handleToggleSelectAll}
                  disabled={filteredItems.length === 0}
                  className="w-4 h-4 rounded text-indigo-600 border-slate-300 focus:ring-indigo-500 cursor-pointer disabled:opacity-40"
                  aria-label="Select All Backorder Products"
                />
              </div>
              ...
              {/* Delete Option beside the search bar */}
              {selectedIds.length > 0 && (
                <button
                  type="button"
                  id="delete-selected-backorder-btn"
                  onClick={() => setShowDeleteConfirm(true)}
                  disabled={deleting}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold text-xs sm:text-sm shadow-xs transition-colors cursor-pointer shrink-0 disabled:opacity-50"
                >
                  {deleting ? (
      ...
      {/* Confirmation Modal for Deleting Selected Items */}
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-4 border border-slate-200">
            ...
            <p className="text-sm text-slate-600">
              ...
              <span className="font-bold text-slate-900">{selectedIds.length}</span> selected backorder product record
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 text-xs font-bold rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 text-xs font-bold rounded-lg bg-red-600 hover:bg-red-700 text-white transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
